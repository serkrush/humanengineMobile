...
import { IIdentity } from "server/acl/types";
...
export default class Model {
    ...
    private static mIdentity: IIdentity;
    ...
    public static get saga() { return Model.mSagas; }
    public get entityName() {return this.mEntityName; }
    public static get store() { return Model.mStore; }
    public static set store(store:Store) { Model.mStore = store; }
    public static get identity() { return Model.mIdentity; }
    public static set identity(identity:IIdentity) { Model.mIdentity = identity; }
    ...
    protected callApi(endpoint,  method = 'GET', data = {}) {
        ...
        return fetch(fullUrl, params).then((response) => {
            ...
            const norma = normalize(camelizeKeys(json.data), this.mSchema);
            const entityData = json.hasOwnProperty('data') ? norma : {};
    ...
    public getPagerItems(pageName: string = null) {
        if (Model.store) {
            const name = pageName ? pageName : this.mEntityName;
            const state = Model.store.getState();
            const pager = state.pagination;
            const entities = state.entities;
            if (pager.has(name) && entities.has(this.mEntityName)) {
                const pageNumber = pager.getIn([name, 'currentPage']);
                if (pager.hasIn([name, 'pages', pageNumber, 'ids'])) {
                    const items = entities.get(this.mEntityName);
                    const ids = pager.getIn([name, 'pages', pageNumber, 'ids']);
    ...
    public * pageEntity(func, params) {
        const pageName  = "pageName" in params?params.pageName:this.mEntityName;
        yield put(pageFetching(pageName, params.page, true, params.force));       // send event about starting page fetching
            
        const pagination = yield select(state => state.pagination);         // check if this page already fetched
